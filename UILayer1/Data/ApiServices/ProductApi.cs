...
namespace UIlayer.Data.ApiServices
{
    public class ProductApi
    {
        ...
        /*public  IEnumerable<Product> GetProduct()
        {
            ...
            {
                _responseModel = null;
                string url = Configuration.GetSection("urlLink").ToString();
                Uri uri = new Uri(url);
        ...
        public  Product GetProduct(int id)
        {
            ...
            using (HttpClient httpclient = new HttpClient())
            {
                string url = Configuration.GetSection("urlLink").ToString()+id;
                Uri uri = new Uri(url);
                System.Threading.Tasks.Task<HttpResponseMessage> result = httpclient.GetAsync(uri);
                if (result.Result.IsSuccessStatusCode)
                {
        ...
        public bool CreateProduct(Product product)
        {
            using (HttpClient httpclient = new HttpClient())
            {
                string data = Newtonsoft.Json.JsonConvert.SerializeObject(product);
                StringContent content = new StringContent(data, Encoding.UTF8, "application/json");
                string url = Configuration.GetSection("Development")["ApiBaseUrl"].ToString()+ "api/product";
